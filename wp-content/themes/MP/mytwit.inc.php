...
class MyTwit {
	...
	*/
	protected function intoRelativeTime($created_at) {
		...
		if (($seconds / 60 / 60 / 24) > 1) return $this->formatPlural(round($seconds / 60 / 60 / 24),'d');
		elseif (($seconds / 60 / 60) > 1) return $this->formatPlural(round($seconds / 60 / 60),'h');
		else if (($seconds / 60 ) > 1) return $this->formatPlural(round($seconds / 60),'m');
		else return round($seconds).'s';
	}
