...
function remove_contact_autop(){
if(is_page())
{
	global $post;
	$page_content = $post -> post_content;
	if(strpos($page_content, 'form_container') !== false)
	{		
		remove_filter('the_content', 'wpautop');
	}
}
}
add_action('wp_head', 'remove_contact_autop');
...
add_filter( 'wp_video_shortcode', 'custom_video_wrap', 10, 4 ) ;

function custom_video_wrap($html, $url, $attr, $post_ID) {    
	return str_replace('<video', '<video width="100%" height="100%"', $html);
